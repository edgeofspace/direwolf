 ...
 */
    ...
    /* Set defaults for the frequency mappings */
    for (m = 0; m < MAX_FREQMAP; m++) {
        p_misc_config->freqmap[m].sdr = 0;
        p_misc_config->freqmap[m].channel = 0;
        p_misc_config->freqmap[m].freq = 0;
    }
...
/*
 * INSTANCE             - Direwolf instance number 
 */
      else if (strcasecmp(t, "INSTANCE") == 0) {
          int n;
          t = split(NULL,0);
          if (t == NULL) {
            text_color_set(DW_COLOR_ERROR);
            dw_printf ("Line %d: Missing instance number for INSTANCE command.\n", line);
            continue;
          }
          n = atoi(t);
          if (n >= 0 && n <= 10) {
              p_misc_config->direwolf_instance = n;
          }
          else {
              p_misc_config->direwolf_instance = 0;
              text_color_set(DW_COLOR_ERROR);
              dw_printf ("Line %d: Invalid instance number for Direwolf INSTANCE number. Using %d.\n",
                      line, p_misc_config->direwolf_instance);
          }
        }

/*
 * FREQMAP             - Map of SDR to Frequency to Channel
 *
 * This should be a list of tuples, space delimited:  SDR# CHANNEL# FREQ_IN_HZ ....
 * For example, this lists two mappings:
 * FREQMAP 0 0 144390000 0 2 144905000
 */
      else if (strcasecmp(t, "FREQMAP") == 0) {
          int n;
          int param_counter = 0;
          int tuple_counter = 0;
          ...
          if (t == NULL) {
            text_color_set(DW_COLOR_ERROR);
            dw_printf ("Line %d: Missing parameters for FREQMAP command.\n", line);
            continue;
          }

          // Loop through all of the parameters on this line
          while (t != NULL) {
              // Convert this value to a integer.  We don't care if it was negative or not.  ;)
              n = abs(atoi(t));

              // Increment the tuple_counter for each set of 3 parameters encountered
              // This is basically the start of a new tuple within the params listed on this line
              if (param_counter > 0 && param_counter % 3 == 0) 
                  tuple_counter++;

              // Assign this parameter to the proper value in the tuple
              switch(param_counter - tuple_counter * 3) {
                  case 0:
                      p_misc_config->freqmap[tuple_counter].sdr = n;
                      break;
                  case 1:
                      p_misc_config->freqmap[tuple_counter].channel = n;
                      break;
                  case 2:
                      p_misc_config->freqmap[tuple_counter].freq = n;
                      break;
                  default:
                      break;
              }
              ...
              // Grab the next value on this line
              t = split(NULL, 0);
          }
      }
...
/*
 * PGHOST               - Postgresql hostname for database connections
 */
        else if (strcasecmp(t, "PGHOST") == 0) {
          t = split(NULL,0);
          if (t == NULL) {
            text_color_set(DW_COLOR_ERROR);
            dw_printf ("Config file: Missing database hostname for PGHOST on line %d.  Skipping PGHOST configuration.\n", line);
            strlcpy (p_misc_config->pghost, "\0", sizeof("\0"));
            continue;
          }
          else {
            strlcpy (p_misc_config->pghost, t, sizeof(p_misc_config->pghost));
          }
        }

/*
 * PGPASSWORD               - Postgresql hostname for database connections
 */
        else if (strcasecmp(t, "PGPASSWORD") == 0) {
          t = split(NULL,0);
          if (t == NULL) {
            text_color_set(DW_COLOR_ERROR);
            dw_printf ("Config file: Missing database password for PGPASSWORD on line %d.  Continuing without database password.\n", line);
            strlcpy (p_misc_config->pgpassword, "\0", sizeof("\0"));
            continue;
          }
          else {
            strlcpy (p_misc_config->pgpassword, t, sizeof(p_misc_config->pgpassword));
          }
        }

/*
 * PGPORT               - Postgresql TCP port number for database connections
 */

      else if (strcasecmp(t, "PGPORT") == 0) {
          int n;
          t = split(NULL,0);
          if (t == NULL) {
            text_color_set(DW_COLOR_ERROR);
            dw_printf ("Line %d: Missing port number for PGPORT.  Skipping PGPORT config.\n", line);
            continue;
          }
          n = atoi(t);
          if (n >= 0 && n <= 65534) {
              p_misc_config->pgport = n;
          }
          else {
              p_misc_config->pgport = 0;
              text_color_set(DW_COLOR_ERROR);
              dw_printf ("Line %d: Skipping PGPORT configuration.\n",
                      line);
          }
        }
...
/*
 * PGUSER               - Postgresql username for database connections
 */
        else if (strcasecmp(t, "PGUSER") == 0) {
          t = split(NULL,0);
          if (t == NULL) {
            text_color_set(DW_COLOR_ERROR);
            dw_printf ("Config file: Missing database username for PGUSER on line %d.\n", line);
            continue;
          }
          else {
            strlcpy (p_misc_config->pguser, t, sizeof(p_misc_config->pguser));
          }
        }


/*
 * PGDBNAME             - Postgresql database name for database connections
 */

        else if (strcasecmp(t, "PGDBNAME") == 0) {
          t = split(NULL,0);
          if (t == NULL) {
            text_color_set(DW_COLOR_ERROR);
            dw_printf ("Config file: Missing database name for PGDBNAME on line %d.\n", line);
            continue;
          }
          else {
            strlcpy (p_misc_config->pgdbname, t, sizeof(p_misc_config->pgdbname));
          }
        }
